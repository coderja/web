...
#查询所有员工的入职日期、部门编号、姓名
SELECT depId,hideDate,ename FROM emp;
...
#名称用薪水代替
SELECT ename AS 姓名,(salary+500)*12+5000 AS 薪水 FROM emp;
...
SELECT * FROM emp  WHERE depId!=10; 
#查询尚未确定部门的员工信息
SELECT * FROM emp WHERE depId=NULL;
...
SELECT * FROM emp WHERE BETWEEN '2000-12-31' AND '2000-1-1';
#查询出不在2000年入职的员工的所有信息
SELECT * FROM emp WHERE hideDate<='2000-1-1' OR hideDate>='2000-12-31';
#查询出部门编号为10,30,50的员工所有信息
SELECT * FROM emp WHERE depId=10 OR depId=30 OR depId=50;
...
#查询出工资大于6000的员工数量
SELECT COUNT(salary) AS 总数 FROM emp  WHERE salary>6000;
#查询出10号部门工资的工资总和
SELECT sum(salary) FROM  emp WHERE depId=10;  